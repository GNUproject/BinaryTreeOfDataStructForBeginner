...
void CreateBinaryTree(Tree * const BinaryTree, const ElemNumb Max, const char * const FileName)
{
	...
	else
	{
		...
		const size_t FullDLRLen = strlen(TempDLR) + 1;
		const size_t FullLDRLen = strlen(TempLDR) + 1;
		const size_t FullLRDLen = strlen(TempLRD) + 1;
		const size_t FullFileNameLen = strlen(TempFile) + 1;
...
static void PreOrderTraversal(const Tree * const BinaryTree, FILE * const Stream)
{
	...
	while (temp || S.Size)
	{
		...
		while (temp) // 一直向左并将沿途结点压入堆栈
		{
			...
			Push(&S, temp);
#ifdef LOG
			TextFileOperationWithStack(&S, APPEND);
#endif
			temp = temp->Left;
		}
...
#ifdef LOG
				TextFileOperationWithStack(&Q, APPEND);
#endif
				T = T->Right;
			}
...
static void LevelOrderTraversal(const Tree * const BinaryTree, FILE * const Stream)
{
	...
	if (BinaryTree->Size)
	{
		...
		Enqueue(&Link, BinaryTree->Root); // 入队
#ifdef LOG
		TextFileOperationWithQueue(&Link, APPEND);
#endif
		LinkItem temp;
		ElemNumb n = 2;
		TreeNode NullItem = {
