...
void CreateQueue(LinkList * const Link, const ElemNumb Max)
{
	// if (Link == NULL)
	if (!Link)
...
#ifdef LOG
// 文件追加操作
void TextFileOperationWithQueue(LinkList * const Link, const char * const FileName, const char * const Mode)
{
	// if (Link == NULL || FileName == NULL || Mode == NULL)
	if (!Link || !FileName || !Mode)
		UnusualToExit(NullPointer, NULL);
